<div class="admin-users self-center" wire:ignore>
        ...
        @foreach ($users as $user)
            <tr>
                ...
                <td>
                    ...
                    @elseif (auth('user')->user()->id === $user->id)
                        <span>
                            {{ $user->role }}
                        </span>
                    ...
                    @else
                        <a class="role" href="#" aria-label="edit role" onclick="toggleEditRole(event, {{ $user->id }})">
                            {{ $user->role }}
                        </a>
                        <span class="edit-role hidden">
                            <select>
                                <option value="admin" {{ $user->role === "admin" ? "selected" : "" }}>admin</option>
                                <option value="user" {{ $user->role === "user" ? "selected" : "" }}>user</option>
                            </select>
                            <span class="check" onclick="editRole(event, {{ $user->id }})">
                            ...
                            <span class="cancel" onclick="toggleEditRole(event, {{ $user->id }})">
                ...
                <td class="actions">
                    @if (auth('user')->user()->id !== $user->id && auth('user')->user()->role != 'user' && $user->role !== 'super admin')
                        <span class="login relative" onclick="logInto({{ $user->id }})">
                            ...
                            <span class="hover-content nowrap small rounded black top center auto">
                                <small>Se connecter à la place de l'utilisateur</small>
                            </span>
                        </span>
                    @endif
                    @if (auth('user')->user()->id !== $user->id && auth('user')->user()->role != 'user' && $user->role !== 'super admin')
                        <span class="delete relative" onclick="deleteUser(event, {{ $user->id }})">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512" fill="currentColor" width="16" height="16">
...
@script
    <script>
        ...
        document.addEventListener('livewire:initialized', function () {
            ...
            window.toggleEditRole = (e, user_id) => {
                if (! e.currentTarget.classList.contains('cancel')) {
                    ...
                    e.currentTarget.parentElement.previousElementSibling.classList.remove('hidden');
                }

            }

            window.editRole = async (event, user_id) => {
                const select = event.currentTarget.parentElement.querySelector('select');
                const role = select.value;

                const res = await fetch("{{ route('admin.users2.edit-role') }}", {
                    method: 'POST',
                    headers: {
                ...
                if (res.status === 200) {
                    ...
                    select.parentElement.previousElementSibling.innerText = role;
                    select.parentElement.previousElementSibling.classList.remove('hidden');
                }
            }

            window.logInto = async (user_id) => {
                const res = await fetch("{{ route('admin.users2.log-into') }}", {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        "Content-Type": "application/json"
            ...
            window.deleteUser = async (event, user_id) => {
                const tr = event.currentTarget.parentElement.parentElement;
                const res = await fetch("{{ route('admin.users2.delete-user') }}", {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        "Content-Type": "application/json"
